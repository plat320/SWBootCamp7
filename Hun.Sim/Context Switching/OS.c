...
char* _OS_Get_Stack(int size){
	...
	size = (size + 7) & ~0x7; 	// 올림해서 8의 배수로 만듬
...
int OS_Create_Task_Simple(void(*ptask)(void*), void* para, int prio, int size_stack)
{
	...
	// task가 사용 할 stack 할당
	ptcb->top_of_stack = (unsigned long*)_OS_Get_Stack(size_stack);	//이전 Pstack return , 결국 호출 함수의 stack 첫 주소
...
void OS_Scheduler_Start(void)
{
	...
	for(i = 0;i <= 42;i++)
	{
		NVIC_SetPriority(i, 0xe);	// 모든 우선순위 초기화 하나 높은 걸로
	}
